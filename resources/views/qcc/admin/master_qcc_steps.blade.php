...
<div class="animate-reveal">
    ...
    <nav class="flex mb-4 md:mb-6 text-xs md:text-sm text-gray-400">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li class="text-[#091E6E] font-semibold tracking-tight text-[10px] md:text-xs">Master Steps</li>
        </ol>
    </nav>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
        <div>
            <h2 class="text-2xl md:text-3xl font-bold text-[#091E6E]">Master Steps QCC</h2>
            <p class="text-xs md:text-sm text-gray-400">Pengaturan 8 Langkah Standar QCC & Template Dokumen</p>
        </div>
        
        <div class="flex flex-wrap gap-3 w-full md:w-auto justify-start md:justify-end items-center">
            <form action="{{ route('qcc.admin.master_steps') }}" method="GET" id="filterForm" class="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
            ...
            <table class="w-full text-left border-separate border-spacing-y-2 min-w-[900px] md:min-w-full">
                <thead>
                    <tr class="sidebar-gradient shadow-md">
                        ...
                        <th class="px-3 md:px-6 py-3 md:py-4 text-white text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold">No. Step</th>
                        <th class="px-3 md:px-6 py-3 md:py-4 text-white text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold">Nama Langkah</th>
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-blue-50/50 transition-all group shadow-sm border border-gray-100">
                        ...
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100">
                            <div class="w-7 h-7 md:w-9 md:h-9 rounded-lg bg-blue-50 text-[#091E6E] font-bold flex items-center justify-center border border-blue-100 text-[10px] md:text-xs group-hover:bg-[#091E6E] group-hover:text-white transition-all duration-300">
                                {{ $step->step_number }}
                            </div>
                        </td>
                        ...
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100">
                            <div class="flex flex-col leading-tight">
                                <span class="font-bold text-[#091E6E] text-xs md:text-sm group-hover:text-[#130998]">{{ $step->step_name }}</span>
                                <span class="text-[8px] md:text-[10px] text-gray-400 italic line-clamp-1 max-w-xs md:max-w-lg">{{ $step->description ?? 'Tidak ada deskripsi tersedia.' }}</span>
                            </div>
                        </td>
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100">
                            @if($step->template_file_path)
                                <span class="text-[7px] md:text-[9px] bg-emerald-50 text-emerald-600 px-1.5 md:px-2 py-0.5 md:py-1 rounded-md font-bold flex items-center gap-1 w-fit border border-emerald-100">
                                    <i class="fa-solid fa-file-circle-check"></i> TERSEDIA
                                </span>
                            @else
                                <span class="text-[7px] md:text-[9px] bg-gray-50 text-gray-400 px-1.5 md:px-2 py-0.5 md:py-1 rounded-md font-bold flex items-center gap-1 w-fit border border-gray-100">
                        ...
                        <td class="px-3 md:px-6 py-2 md:py-3 rounded-r-xl border-y border-r border-gray-100 text-center">
                            <div class="flex justify-center gap-1 md:gap-2">
                                <button onclick="openDetailModal({{ json_encode($step) }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Detail">
                                    <i class="fa-solid fa-eye text-[8px] md:text-[10px]"></i>
                                </button>
                                <button onclick="openEditModal({{ json_encode($step) }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-500 hover:text-white transition-all shadow-sm" title="Edit">
                                    <i class="fa-solid fa-pen-to-square text-[8px] md:text-[10px]"></i>
                                </button>
                                <button onclick="confirmDelete('{{ $step->id }}')" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-red-50 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all shadow-sm" title="Hapus">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="text-center py-10 text-gray-300 italic text-xs md:text-sm">Belum ada langkah terdaftar.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-lg shadow-2xl animate-reveal overflow-hidden">
            ...
            <div class="p-4 md:p-8 space-y-4 md:space-y-6">
                <div class="flex items-start gap-3 md:gap-4">
                    <div id="det_number" class="text-2xl md:text-4xl font-black text-white bg-[#091E6E] w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg">0</div>
                    <div class="flex-1">
                        <label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Nama Langkah</label>
                ...
                <div class="bg-gray-50 p-4 md:p-6 rounded-xl md:rounded-2xl border border-gray-100 shadow-inner">
                    <label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-2 block">Deskripsi Lengkap</label>
                ...
                <div id="det_file_area" class="hidden">
                    ...
                    <a id="det_download_link" href="#" target="_blank" class="flex items-center gap-2 md:gap-3 p-3 md:p-4 bg-blue-50 border border-blue-100 rounded-lg md:rounded-xl hover:bg-blue-100 transition-all group">
                        <i class="fa-solid fa-file-powerpoint text-red-500 text-base md:text-xl group-hover:scale-110 transition-transform"></i>
    ...
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-lg shadow-2xl animate-reveal overflow-hidden">  
            ...
            <form id="formAdd" action="{{ route('qcc.admin.store_step') }}" method="POST" enctype="multipart/form-data" class="p-4 md:p-8 space-y-4 md:space-y-6">
                ...
                <button type="submit" class="w-full py-3 md:py-4 bg-[#091E6E] text-white rounded-xl font-bold shadow-lg hover:bg-[#130998] transition-all uppercase tracking-widest text-[10px] md:text-xs">Simpan Data</button>
            </form>
        </div>
    </div>
</div>
...
<style>
    ...
    .custom-pagination nav svg { width: 0.875rem; height: 0.875rem; } /* lebih kecil di mobile */
