...
<div class="animate-reveal">
    ...
    <nav class="flex mb-4 md:mb-6 text-xs md:text-sm text-gray-400">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li class="text-[#091E6E] font-semibold tracking-tight text-[10px] md:text-xs">Master Periods</li>
        </ol>
    </nav>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
        <div>
            <h2 class="text-2xl md:text-3xl font-bold text-[#091E6E]">Master Periods QCC</h2>
            <p class="text-xs md:text-sm text-gray-400">Kelola durasi periode dan deadline langkah QCC</p>
        </div>
        
        <div class="flex flex-wrap gap-3 w-full md:w-auto justify-start md:justify-end items-center">
            <form action="{{ route('qcc.admin.master_periods') }}" method="GET" id="filterForm" class="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
    ...
    <!-- Table Section -->
    <div class="glass-card rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 shadow-sm">
            ...
            <table class="w-full text-left border-separate border-spacing-y-2 min-w-[900px] md:min-w-full">
                <thead>
                    <tr class="sidebar-gradient shadow-md">
                        ...
                        <th class="px-3 md:px-6 py-3 md:py-4 text-white text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold text-center">Tahun</th>
                        <th class="px-3 md:px-6 py-3 md:py-4 text-white text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold">Periode & Durasi</th>
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:shadow-md transition-all group shadow-sm border border-gray-100">
                        ...
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100 text-center font-black text-[#091E6E] text-xs md:text-sm">
                            {{ $period->year }}
                        </td>

                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100">
                            <div class="flex flex-col">
                                <span class="font-bold text-[#091E6E] text-xs md:text-sm">{{ $period->period_name }} ({{ $period->period_code }})</span>
                                <span class="text-[9px] md:text-[11px] text-gray-400 font-medium">
                        ...
                        <td class="px-3 md:px-6 py-2 md:py-3 rounded-r-xl border-y border-r border-gray-100 text-center">
                            <div class="flex justify-center gap-1 md:gap-2 flex-wrap">
                                <button onclick="openDetailModal({{ json_encode($period) }}, {{ json_encode($period->periodSteps) }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="View Detail">
                                    <i class="fa-solid fa-eye text-[8px] md:text-[10px]"></i>
                                </button>
                                <button onclick="openDeadlineModal({{ json_encode($period) }}, {{ json_encode($period->periodSteps) }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-600 hover:text-white transition-all shadow-sm" title="Set Deadlines">
                                    <i class="fa-solid fa-list-check text-[8px] md:text-[10px]"></i>
                                </button>
                                <button onclick="openEditModal({{ $period }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-500 hover:text-white transition-all shadow-sm" title="Edit">
                                    <i class="fa-solid fa-pen-to-square text-[8px] md:text-[10px]"></i>
                                </button>
                                <button onclick="confirmDelete('{{ $period->id }}')" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-red-50 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all shadow-sm" title="Hapus">
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="text-center py-10 text-gray-300 italic text-xs md:text-sm">Belum ada periode.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-2xl shadow-2xl animate-reveal overflow-hidden">
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-circle-info mr-2"></i>
                    Detail Periode
                </h3>
            ...
            <div class="p-4 md:p-8 space-y-4 md:space-y-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 bg-gray-50 p-4 md:p-6 rounded-xl md:rounded-2xl border border-gray-100 shadow-inner font-medium">
                    <div>
                        <label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest">Nama & Kode</label>
                        <p id="det_name" class="text-[#091E6E] font-bold text-xs md:text-sm"></p>
                    </div>
                    <div>
                        <label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest">Tahun</label>
                        <p id="det_year" class="text-[#091E6E] font-bold text-xs md:text-sm"></p>
                    </div>
                    <div>
                        <label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest">Durasi Periode</label>
                        <p id="det_range" class="text-[#091E6E] text-xs md:text-sm font-bold"></p>
                    </div>
                    <div>
                        <label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Status</label>
                        <span id="det_status" class="px-2 md:px-3 py-0.5 md:py-1 rounded-full text-[8px] md:text-[10px] font-bold border"></span>
                    </div>
                </div>
                <div class="border-t pt-3 md:pt-4">
                    <h4 class="text-[10px] md:text-xs font-bold text-[#091E6E] uppercase tracking-widest mb-3 md:mb-4">Target Deadline Per Langkah</h4>
                    <div id="det_steps" class="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
                        <!-- Content via JS -->
                    </div>
                </div>
                <button onclick="closeModal('modalDetail')" class="w-full py-3 md:py-4 bg-gray-300 text-gray-500 rounded-xl font-bold uppercase tracking-widest text-[10px] md:text-xs hover:bg-gray-200 transition-all">Tutup</button>
            </div>
        </div>
    </div>
</div>
...
<div id="modalAdd" class="fixed inset-0 z-[100] hidden overflow-y-auto bg-black/50 backdrop-blur-sm">
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-xl shadow-2xl animate-reveal overflow-hidden">  
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-calendar-plus mr-2"></i>
                    Tambah Periode Baru
                </h3>
            ...
            <form id="formAdd" action="{{ route('qcc.admin.store_period') }}" method="POST" class="p-4 md:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                ...
                <div class="col-span-1 sm:col-span-2 pt-2 md:pt-4">
                    <p class="text-[8px] md:text-[10px] text-gray-400 italic">* Status periode awal akan otomatis diset <span class="font-bold text-emerald-500">ACTIVE</span>.</p>
                </div>
                <button type="submit" class="col-span-1 sm:col-span-2 py-3 md:py-4 bg-[#091E6E] text-white rounded-xl font-bold shadow-lg hover:bg-[#130998] transition-all uppercase tracking-widest text-[10px] md:text-xs">Simpan Data Periode</button>
            </form>
        </div>
    </div>
</div>
    ...
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2.5rem] w-full max-w-xl shadow-2xl animate-reveal overflow-hidden">
            ...
            <form id="formEdit" method="POST" class="p-4 md:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                ...
                <div class="col-span-1 sm:col-span-2">
                    <label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Status Periode</label>
                ...
                <div class="flex flex-col sm:flex-row gap-3 col-span-1 sm:col-span-2 mt-2 md:mt-4">
                    <button type="button" onclick="closeModal('modalEdit')" class="flex-1 py-3 md:py-4 bg-gray-300 text-gray-500 rounded-xl font-bold uppercase tracking-widest text-[9px] md:text-[10px]">Batal</button>
                    <button type="submit" class="flex-1 py-3 md:py-4 bg-amber-500 text-white rounded-xl font-bold shadow-lg uppercase tracking-widest text-[9px] md:text-[10px] hover:bg-amber-600 transition-all">Update Data</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="modalDeadline" class="fixed inset-0 z-[100] hidden overflow-y-auto bg-black/50 backdrop-blur-sm">
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-2xl shadow-2xl animate-reveal overflow-hidden">
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-sliders mr-2"></i>
                    Set Deadlines Langkah
                </h3>
                <button onclick="closeModal('modalDeadline')" class="text-white/70 hover:text-white text-xl md:text-2xl">&times;</button>
            </div>
            <form id="formDeadline" method="POST" class="p-4 md:p-8">
                @csrf @method('PUT')
                <div class="mb-3 md:mb-4">
                    <h4 id="deadlinePeriodName" class="text-[#091E6E] font-bold text-sm md:text-lg"></h4>
                </div>
                <div class="overflow-x-auto border rounded-xl md:rounded-2xl overflow-hidden mb-4 md:mb-6">
                    <table class="w-full text-left">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-3 md:px-4 py-2 md:py-3 text-[8px] md:text-[10px] font-bold text-gray-400 uppercase">Step QCC</th>
                                <th class="px-3 md:px-4 py-2 md:py-3 text-[8px] md:text-[10px] font-bold text-gray-400 uppercase text-center">Batas Akhir (Deadline)</th>
                            </tr>
                        </thead>
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <button type="button" onclick="closeModal('modalDeadline')" class="flex-1 py-3 md:py-4 bg-gray-300 text-gray-500 rounded-xl font-bold uppercase tracking-widest text-[9px] md:text-xs">Tutup</button>
                    <button type="submit" class="flex-1 py-3 md:py-4 bg-[#091E6E] text-white rounded-xl font-bold shadow-lg uppercase tracking-widest text-[9px] md:text-xs hover:bg-[#130998]">Update Deadline</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function openDetailModal(period, periodSteps) {
        ...
        periodSteps.forEach(ps => {
            stepsCont.innerHTML += `
                <div class="flex justify-between items-center bg-white p-2 md:p-3 rounded-lg md:rounded-xl border border-gray-100">
                    <span class="font-bold text-[#091E6E] text-xs md:text-sm">Step ${ps.step.step_number}</span>
                    <span class="text-gray-400 font-bold tracking-tighter text-[9px] md:text-xs">${ps.deadline_date}</span>
                </div>
            `;
        });
            ...
            list.innerHTML += `
                <tr class="border-t border-gray-100">
                    <td class="px-3 md:px-4 py-2 md:py-3 whitespace-nowrap">
                        <span class="text-[10px] md:text-xs font-bold text-[#091E6E]">Step ${ps.step.step_number}</span><br>
