...
<div class="animate-reveal">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
        <div>
            <h2 class="text-2xl md:text-3xl font-bold text-[#091E6E]">Master Section</h2>
            <p class="text-xs md:text-sm text-gray-400">Manajemen data section per departemen</p>
        </div>

        <div class="flex flex-wrap gap-3 w-full md:w-auto justify-start md:justify-end items-center">
            <form action="{{ route('admin.master_sections.index') }}" method="GET" class="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
                ...
                <div class="relative w-full sm:w-64">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kode, nama, departemen..."
            ...
            <button onclick="openModal('modalAdd')" class="bg-[#091E6E] hover:bg-[#130998] text-white px-4 md:px-5 py-2 rounded-xl flex items-center gap-2 shadow-lg transition-all active:scale-95 text-[10px] md:text-xs font-bold uppercase tracking-wider">
                <i class="fa-solid fa-layer-group"></i> Tambah Section
            </button>
        </div>
    </div>
    ...
    <div class="glass-card rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 shadow-sm border border-white">
            ...
            <table class="w-full text-left border-separate border-spacing-y-2 min-w-[900px] md:min-w-full">
                <thead>
                    <tr class="sidebar-gradient shadow-md">
                        ...
                        <th class="px-3 md:px-6 py-3 md:py-4 text-white text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold text-center">NPK / Admin</th>
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-blue-50/50 transition-all shadow-sm border border-gray-100">
                        ...
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100">
                            <p class="font-bold text-[#091E6E] text-xs md:text-sm">{{ $section->name }}</p>
                            <p class="text-[8px] md:text-[10px] text-gray-400 font-bold uppercase tracking-tighter">Kode: {{ $section->code }}</p>
                        </td>
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100 text-xs md:text-sm text-gray-600">{{ $section->alias ?: '-' }}</td>
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100 text-center">
                            <span class="text-[8px] md:text-[10px] bg-blue-50 text-blue-700 px-2 md:px-3 py-0.5 md:py-1 rounded-lg font-bold border border-blue-100">{{ $section->code_department }}</span>
                            @if($section->department)
                                <p class="text-[8px] text-gray-400 mt-1">{{ $section->department->name }}</p>
                            @endif
                        </td>
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100 text-center text-[10px] md:text-xs text-gray-600">
                            <div>{{ $section->npk ?: '-' }}</div>
                            <div class="text-[8px] text-gray-400">Admin: {{ $section->npk_admin ?: '-' }}</div>
                        </td>
                        <td class="px-3 md:px-6 py-2 md:py-3 rounded-r-xl border-y border-r border-gray-100 text-center">
                            <div class="flex justify-center gap-1 md:gap-2">
                                <button onclick="openDetailModal({{ json_encode($section) }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm"><i class="fa-solid fa-eye text-[8px] md:text-[10px]"></i></button>
                                <button onclick="openEditModal({{ json_encode($section) }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-500 hover:text-white transition-all shadow-sm"><i class="fa-solid fa-pen-to-square text-[8px] md:text-[10px]"></i></button>
                                <button onclick="confirmDelete('{{ $section->id }}', '{{ $section->code }}', '{{ addslashes($section->name) }}')" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-red-50 text-red-500 rounded-lg hover:bg-red-600 hover:text-white transition-all shadow-sm"><i class="fa-solid fa-trash text-[8px] md:text-[10px]"></i></button>
                                <form id="delete-form-{{ $section->id }}" action="{{ route('admin.master_sections.destroy', $section->id) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="text-center py-10 md:py-20">
                            <div class="flex flex-col items-center gap-2 text-gray-300">
                                ...
                                <span class="italic text-xs md:text-sm">Data section tidak ditemukan...</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-between items-center gap-4 border-t border-gray-50 pt-4 md:pt-6">
            ...
            <div class="custom-pagination">{{ $sections->links('pagination::tailwind') }}</div>
        </div>
    </div>
</div>
    ...
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-lg shadow-2xl animate-reveal overflow-hidden">
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-layer-group mr-2"></i> Detail Section</h3>
            ...
            <div class="p-4 md:p-8 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    <div><label class="text-[10px] font-bold text-gray-400 uppercase">Departemen</label><p id="det_department" class="text-[#091E6E] font-bold text-sm"></p></div>
                    <div><label class="text-[10px] font-bold text-gray-400 uppercase">NPK</label><p id="det_npk" class="text-[#091E6E] font-bold text-sm"></p></div>
                    <div><label class="text-[10px] font-bold text-gray-400 uppercase">NPK Admin</label><p id="det_npk_admin" class="text-[#091E6E] font-bold text-sm"></p></div>
                </div>
...
<div id="modalAdd" class="fixed inset-0 z-[100] hidden overflow-y-auto bg-black/50 backdrop-blur-sm">
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-xl shadow-2xl animate-reveal overflow-hidden">
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-layer-group mr-2"></i> Tambah Section</h3>
                <button onclick="closeModal('modalAdd')" class="text-white/70 hover:text-white text-xl md:text-2xl">&times;</button>
            </div>
            <form id="formAdd" action="{{ route('admin.master_sections.store') }}" method="POST" class="p-4 md:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                @csrf
                <div>
                ...
                <div class="sm:col-span-2">
                    <label class="text-[10px] font-bold text-gray-400 uppercase">Nama Section <span class="text-red-500">*</span></label>
                ...
                <div class="sm:col-span-2">
                    <label class="text-[10px] font-bold text-gray-400 uppercase">Departemen <span class="text-red-500">*</span></label>
                    <select name="code_department" required class="w-full mt-1 px-3 py-2 bg-gray-50 border rounded-lg outline-none text-sm font-semibold">
                        <option value="">-- Pilih Departemen --</option>
                        @foreach($departments as $dept)
                            <option value="{{ $dept->code }}" @selected(old('code_department') === $dept->code)>{{ $dept->code }} - {{ $dept->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="sm:col-span-2">
                    <label class="text-[10px] font-bold text-gray-400 uppercase">NPK Admin <span class="text-red-500">*</span></label>
                    <input type="text" name="npk_admin" value="{{ old('npk_admin') }}" required maxlength="6" class="w-full mt-1 px-3 py-2 bg-gray-50 border rounded-lg focus:ring-2 focus:ring-[#091E6E] outline-none text-sm">
                </div>
                <button type="submit" class="sm:col-span-2 py-3 bg-[#091E6E] text-white rounded-xl font-bold shadow-lg uppercase text-xs">Simpan Section</button>
            </form>
        </div>
    </div>
</div>
...
<div id="modalEdit" class="fixed inset-0 z-[100] hidden overflow-y-auto bg-black/50 backdrop-blur-sm">
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-xl shadow-2xl animate-reveal overflow-hidden">
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-pen-to-square mr-2"></i> Update Section</h3>
            ...
            <form id="formEdit" method="POST" class="p-4 md:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    <label class="text-[10px] font-bold text-gray-400 uppercase">Departemen <span class="text-red-500">*</span></label>
                    <select name="code_department" id="edit_code_department" required class="w-full mt-1 px-3 py-2 bg-gray-50 border rounded-lg outline-none text-sm font-semibold">
                        @foreach($departments as $dept)
                ...
                <div class="sm:col-span-2">
                    <label class="text-[10px] font-bold text-gray-400 uppercase">NPK Admin <span class="text-red-500">*</span></label>
                    <input type="text" name="npk_admin" id="edit_npk_admin" required maxlength="6" class="w-full mt-1 px-3 py-2 bg-gray-50 border rounded-lg focus:ring-2 focus:ring-amber-500 outline-none text-sm">
                </div>
...
<script>
    const masterUrl = @json(url('/admin/master-sections'));
    ...
    function openDetailModal(item) {
        ...
        document.getElementById('det_department').innerText = item.code_department + (item.department ? ' - ' + item.department.name : '');
        document.getElementById('det_npk').innerText = item.npk || '-';
        document.getElementById('det_npk_admin').innerText = item.npk_admin || '-';
    ...
    function openEditModal(item) {
        ...
        document.getElementById('edit_code_department').value = item.code_department;
        document.getElementById('edit_npk').value = item.npk || '';
        document.getElementById('edit_npk_admin').value = item.npk_admin || '';
        openModal('modalEdit');
    }

    function confirmDelete(id, code, name) {
        confirmDeleteMaster(id, 'Hapus Section?', `Apakah Anda yakin ingin menghapus section <b>${code}</b> - ${name}?<br><small class="text-red-500 italic">Section yang masih memiliki sub section atau karyawan tidak dapat dihapus.</small>`);
    }

    bindMasterFormConfirm('formAdd', 'Simpan Section?', 'Pastikan semua field terisi dengan benar.');
    bindMasterFormConfirm('formEdit', 'Update Section?', 'Perubahan data section akan disimpan secara permanen.', '#F59E0B', 'Ya, Update!');
