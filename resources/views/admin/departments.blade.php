...
<div class="animate-reveal">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
        <div>
            <h2 class="text-2xl md:text-3xl font-bold text-[#091E6E]">Master Departemen</h2>
            <p class="text-xs md:text-sm text-gray-400">Manajemen data departemen organisasi PT Aisin Indonesia Automotive</p>
        </div>

        <div class="flex flex-wrap gap-3 w-full md:w-auto justify-start md:justify-end items-center">
            <form action="{{ route('admin.master_departments.index') }}" method="GET" class="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
                ...
                <div class="relative w-full sm:w-64">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kode, nama, alias, NPK..."
            ...
            <button onclick="openModal('modalAdd')" class="bg-[#091E6E] hover:bg-[#130998] text-white px-4 md:px-5 py-2 rounded-xl flex items-center gap-2 shadow-lg transition-all active:scale-95 text-[10px] md:text-xs font-bold uppercase tracking-wider">
                <i class="fa-solid fa-building"></i> Tambah Departemen
            </button>
        </div>
    </div>
    ...
    <div class="glass-card rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-6 shadow-sm border border-white">
            ...
            <table class="w-full text-left border-separate border-spacing-y-2 min-w-[800px] md:min-w-full">
                <thead>
                    <tr class="sidebar-gradient shadow-md">
                        ...
                        <th class="px-3 md:px-6 py-3 md:py-4 text-white text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold text-center">Division</th>
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-blue-50/50 transition-all shadow-sm border border-gray-100">
                        ...
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100">
                            <p class="font-bold text-[#091E6E] text-xs md:text-sm">{{ $dept->name }}</p>
                            <p class="text-[8px] md:text-[10px] text-gray-400 font-bold uppercase tracking-tighter">Kode: {{ $dept->code }}</p>
                        </td>
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100 text-xs md:text-sm text-gray-600">{{ $dept->alias ?: '-' }}</td>
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100 text-center">
                            <span class="text-[8px] md:text-[10px] bg-blue-50 text-blue-700 px-2 md:px-3 py-0.5 md:py-1 rounded-lg font-bold border border-blue-100">{{ $dept->code_division }}</span>
                            @if($dept->division)
                                <p class="text-[8px] text-gray-400 mt-1">{{ $dept->division->name }}</p>
                            @endif
                        </td>
                        <td class="px-3 md:px-6 py-2 md:py-3 border-y border-gray-100 text-center text-xs md:text-sm text-gray-600">{{ $dept->npk ?: '-' }}</td>
                        <td class="px-3 md:px-6 py-2 md:py-3 rounded-r-xl border-y border-r border-gray-100 text-center">
                            <div class="flex justify-center gap-1 md:gap-2">
                                <button onclick="openDetailModal({{ json_encode($dept) }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Detail">
                                    <i class="fa-solid fa-eye text-[8px] md:text-[10px]"></i>
                                </button>
                                <button onclick="openEditModal({{ json_encode($dept) }})" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-amber-50 text-amber-600 rounded-lg hover:bg-amber-500 hover:text-white transition-all shadow-sm" title="Edit">
                                    <i class="fa-solid fa-pen-to-square text-[8px] md:text-[10px]"></i>
                                </button>
                                <button onclick="confirmDelete('{{ $dept->id }}', '{{ $dept->code }}', '{{ addslashes($dept->name) }}')" class="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-red-50 text-red-500 rounded-lg hover:bg-red-600 hover:text-white transition-all shadow-sm" title="Hapus">
                                    <i class="fa-solid fa-trash text-[8px] md:text-[10px]"></i>
                                </button>
                                <form id="delete-form-{{ $dept->id }}" action="{{ route('admin.master_departments.destroy', $dept->id) }}" method="POST" class="hidden">
                                    @csrf @method('DELETE')
                                </form>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-10 md:py-20">
                            <div class="flex flex-col items-center gap-2 text-gray-300">
                                ...
                                <span class="italic text-xs md:text-sm">Data departemen tidak ditemukan...</span>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="mt-4 md:mt-6 flex flex-col md:flex-row justify-between items-center gap-4 border-t border-gray-50 pt-4 md:pt-6">
            ...
            <div class="custom-pagination">{{ $departments->links('pagination::tailwind') }}</div>
        </div>
    </div>
</div>
    ...
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-lg shadow-2xl animate-reveal overflow-hidden">
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-building mr-2"></i> Detail Departemen</h3>
            ...
            <div class="p-4 md:p-8 space-y-4 md:space-y-6">
                <div>
                    <h4 id="det_name" class="text-xl md:text-2xl font-bold text-[#091E6E]"></h4>
                    <p id="det_code" class="text-[10px] md:text-sm text-gray-400 font-bold uppercase tracking-widest"></p>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 bg-gray-50 p-4 md:p-6 rounded-xl md:rounded-2xl border border-gray-100 shadow-inner font-medium">
                    <div><label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest">Alias</label><p id="det_alias" class="text-[#091E6E] font-bold text-xs md:text-sm"></p></div>
                    <div><label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest">Division</label><p id="det_division" class="text-[#091E6E] font-bold text-xs md:text-sm"></p></div>
                    <div class="col-span-1 sm:col-span-2"><label class="text-[8px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest">NPK Ka. Dept</label><p id="det_npk" class="text-[#091E6E] font-bold text-xs md:text-sm"></p></div>
                </div>
...
<div id="modalAdd" class="fixed inset-0 z-[100] hidden overflow-y-auto bg-black/50 backdrop-blur-sm">
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-xl shadow-2xl animate-reveal overflow-hidden">
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-building mr-2"></i> Tambah Departemen</h3>
                <button onclick="closeModal('modalAdd')" class="text-white/70 hover:text-white text-xl md:text-2xl">&times;</button>
            </div>
            <form id="formAdd" action="{{ route('admin.master_departments.store') }}" method="POST" class="p-4 md:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                @csrf
                <div>
                    <label class="text-[10px] font-bold text-gray-400 uppercase">Kode <span class="text-red-500">*</span></label>
                    <input type="text" name="code" value="{{ old('code') }}" required maxlength="50" pattern="[A-Za-z0-9._-]+" placeholder="Contoh: HRG" class="w-full mt-1 px-3 py-2 bg-gray-50 border rounded-lg focus:ring-2 focus:ring-[#091E6E] outline-none text-sm font-bold uppercase">
                </div>
                <div>
                    <label class="text-[10px] font-bold text-gray-400 uppercase">NPK Ka. Dept <span class="text-red-500">*</span></label>
                    <input type="text" name="npk" value="{{ old('npk') }}" required maxlength="6" placeholder="Contoh: 000009" class="w-full mt-1 px-3 py-2 bg-gray-50 border rounded-lg focus:ring-2 focus:ring-[#091E6E] outline-none text-sm">
                </div>
                <div class="sm:col-span-2">
                    <label class="text-[10px] font-bold text-gray-400 uppercase">Nama Departemen <span class="text-red-500">*</span></label>
                ...
                <div class="sm:col-span-2">
                    <label class="text-[10px] font-bold text-gray-400 uppercase">Division <span class="text-red-500">*</span></label>
                    <select name="code_division" required class="w-full mt-1 px-3 py-2 bg-gray-50 border rounded-lg outline-none text-sm font-semibold">
                        <option value="">-- Pilih Division --</option>
                        @foreach($divisions as $div)
                            <option value="{{ $div->code }}" @selected(old('code_division') === $div->code)>{{ $div->code }} - {{ $div->name }}</option>
                        @endforeach
                    </select>
                </div>
                <button type="submit" class="sm:col-span-2 py-3 md:py-4 bg-[#091E6E] text-white rounded-xl font-bold shadow-lg hover:bg-[#130998] transition-all uppercase tracking-widest text-[10px] md:text-xs active:scale-95">Simpan Departemen</button>
            </form>
        </div>
    </div>
</div>
...
<div id="modalEdit" class="fixed inset-0 z-[100] hidden overflow-y-auto bg-black/50 backdrop-blur-sm">
    <div class="flex items-center justify-center min-h-screen p-2 md:p-4 text-left">
        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] w-full max-w-xl shadow-2xl animate-reveal overflow-hidden">
            <div class="sidebar-gradient p-4 md:p-6 text-white flex justify-between items-center">
                <h3 class="text-base md:text-xl font-bold"><i class="fa-solid fa-pen-to-square mr-2"></i> Update Departemen</h3>
            ...
            <form id="formEdit" method="POST" class="p-4 md:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    <label class="text-[10px] font-bold text-gray-400 uppercase">Division <span class="text-red-500">*</span></label>
                    <select name="code_division" id="edit_code_division" required class="w-full mt-1 px-3 py-2 bg-gray-50 border rounded-lg outline-none text-sm font-semibold">
                        @foreach($divisions as $div)
                            <option value="{{ $div->code }}">{{ $div->code }} - {{ $div->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex flex-col sm:flex-row gap-3 sm:col-span-2 mt-2">
                    ...
                    <button type="submit" class="flex-1 py-3 bg-amber-500 text-white rounded-xl font-bold shadow-lg uppercase tracking-widest text-[10px] hover:bg-amber-600">Update Data</button>
                </div>
            </form>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        const masterUrl = @json(url('/admin/master-departments'));
        ...
        function openDetailModal(item) {
            ...
            document.getElementById('det_division').innerText = item.code_division + (item.division ? ' - ' + item.division.name : '');
        ...
        function openEditModal(item) {
            ...
            document.getElementById('edit_code_division').value = item.code_division;
            document.getElementById('edit_npk').value = item.npk || '';
            openModal('modalEdit');
        }

        function confirmDelete(id, code, name) {
            confirmDeleteMaster(id, 'Hapus Departemen?', `Apakah Anda yakin ingin menghapus departemen <b>${code}</b> - ${name}?<br><small class="text-red-500 italic">Departemen yang masih memiliki section atau data QCC/SS tidak dapat dihapus.</small>`);
        }

        bindMasterFormConfirm('formAdd', 'Simpan Departemen?', 'Pastikan semua field terisi dengan benar.');
        bindMasterFormConfirm('formEdit', 'Update Departemen?', 'Perubahan data departemen akan disimpan secara permanen.', '#F59E0B', 'Ya, Update!');
