        ...
        <div class="p-4 md:p-8">
            ...
            <form action="{{ route('ss.admin.review.store', $submission->id) }}" method="POST" id="formReviewAdmin">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        @foreach($criteria as $key => $label)
                            <div class="flex items-center justify-between gap-3 bg-gray-50/50 border border-gray-200 rounded-xl px-3 py-2">
                                ...
                                <input type="number" name="scores[{{ $key }}]" min="0" max="{{ $criteriaMaxScores[$key] ?? 0 }}" value="{{ old('scores.' . $key, $submission->kdp_scores[$key] ?? 0) }}" class="score-input w-20 px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm font-bold text-[#091E6E] text-center outline-none focus:ring-2 focus:ring-[#091E6E]">
                            </div>
                ...
                <div class="mb-8">
                    ...
                    <textarea name="admin_notes" rows="4" class="w-full border border-gray-200 rounded-xl p-3 focus:ring-2 focus:ring-[#091E6E] bg-gray-50/30 text-sm" placeholder="Tulis alasan persetujuan atau penolakan...">{{ old('admin_notes') }}</textarea>
