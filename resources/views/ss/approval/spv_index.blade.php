...
<div class="animate-reveal">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
        <div>
            <h2 class="text-2xl md:text-3xl font-bold text-[#091E6E]">Review SS — SPV</h2>
        ...
        <form action="{{ route('ss.approval.spv') }}" method="GET" class="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
            ...
            <table class="w-full text-left border-separate border-spacing-y-2 min-w-[800px] md:min-w-full">
                <thead>
                    <tr class="sidebar-gradient shadow-md">
                        ...
                        <th class="px-3 md:px-6 py-3 text-white text-[8px] md:text-[10px] uppercase font-bold text-center">Tanggal</th>
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-blue-50/50 transition-all shadow-sm border border-gray-100">
                        ...
                        <td class="px-3 md:px-6 py-3 border-y text-center text-xs text-gray-600">
                            {{ \Carbon\Carbon::parse($ss->submission_date)->format('d/m/Y') }}
                        </td>
                        <td class="px-3 md:px-6 py-3 rounded-r-xl border-y border-r text-center">
                            <div class="flex flex-wrap justify-center gap-1 md:gap-2">
                                ...
                                @endif
                                <a href="{{ route('ss.approval.spv.review', $ss->id) }}"
                                    class="bg-purple-600 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase hover:bg-purple-700 transition-all">
                                    Review
                                </a>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-16 text-gray-300 italic text-sm">
                            ...
                            Tidak ada ide SS yang menunggu review SPV.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
